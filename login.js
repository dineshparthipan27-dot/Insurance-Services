document.addEventListener("DOMContentLoaded", function () {
    ...
    loginForm.addEventListener("submit", (e) => {
        ...
        if (!role) {
            isValid = false;
            alert("Please select a Portal Access Role to login.");
        }
        ...
        setTimeout(() => {
            if (role === "admin") {
                sessionStorage.setItem("adminLoggedInEmail", email);
    ...
    signupForm.addEventListener("submit", (e) => {
        ...
        if (name.length < 3) {
            showError("signupName", "signupNameError", "Full Name required");
            isValid = false;
        } else {
